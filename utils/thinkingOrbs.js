// Engine de "thinking orbs" — animaciones canvas para estados del chatbot.
// Port de github.com/Jakubantalik/thinking-orbs (src/engine/*).
// 6 estados: working, searching, solving, listening, composing, shaping.
// Presets calibrados para tamaños 64 y 20; otros tamaños usan el preset más cercano.
...
const DARK_FLOOR = 0.32; // brillo minimo (0-1) de los puntos en modo oscuro
...
function paint(ctx, dots, dark, rMin = 0.3) {
  ...
  for (const d of dots) {
    ...
    const raw = dark ? 1 - w : w;
    // Piso de brillo en modo oscuro: sin esto los puntos "lejanos" caen a negro puro
    // y se pierden contra fondos oscuros (#1a1a1a, #2E4053)
    const lit = dark ? DARK_FLOOR + raw * (1 - DARK_FLOOR) : raw;
...
// --- orbits: "working" ---
const drawOrbits = (ctx, size, t, dark, o) => {
...
// --- globe: "searching" ---
const drawGlobe = (ctx, size, t, dark, o) => {
  const spin = 0.5;
  const cx = size / 2, cy = size / 2;
...
const drawRubik = (ctx, size, t, dark, o) => {
  const cx = size / 2, cy = size / 2;
  const R = (size / 2) * 0.82;
...
// --- wave: "listening" ---
const drawWave = (ctx, size, t, dark, o) => {
  const cx = size / 2, cy = size / 2;
...
// --- ribbon: "composing" ---
const drawRibbon = (ctx, size, t, dark, o) => {
  const cx = size / 2, cy = size / 2;
  const R = (size / 2) * 0.78;
  const spin = o.spin ?? 1;
...
const drawMorph = (ctx, size, t, dark, o) => {
...
export const MODE_DRAWS = { orbits: drawOrbits, globe: drawGlobe, rubik: drawRubik, wave: drawWave, ribbon: drawRibbon, morph: drawMorph };
export const STATE_TO_MODE = { working: 'orbits', searching: 'globe', solving: 'rubik', listening: 'wave', composing: 'ribbon', shaping: 'morph' };
export const ST_LABEL = {
  working: 'Trabajando', searching: 'Buscando', solving: 'Resolviendo',
  listening: 'Escuchando', composing: 'Redactando', shaping: 'Generando'
};
...
function scaleCounts(opts, scale) {
  ...
  for (const k of COUNT_KEYS) if (out[k] != null && !done.has(k)) out[k] = Math.max(1, Math.round(out[k] * scale));
...
export function resolvePreset(state, size) {
  ...
  // Presets solo existen para 64 y 20 — tamaños intermedios toman el más cercano
  const presetSize = PRESETS[mode][size] ? size : (size >= 42 ? 64 : 20);
  const key = `${mode}-${presetSize}`;
  if (presetCache.has(key)) return presetCache.get(key);
  const preset = PRESETS[mode][presetSize];
