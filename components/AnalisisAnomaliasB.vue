  ...
  <div class="p-6 space-y-6">
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
      ...
      <div v-else-if="sensoresConAnomalias.length === 0" class="bg-teal-50 border border-teal-200 rounded-lg p-8 text-center">
        ...
        <p class="text-teal-600 mt-1">Todos los sensores de la Bomba B están funcionando normalmente</p>
      </div>
...
const { esBombaBActiva } = useBombaActiva();

// Cuando Bomba B está activa: incluir alertas propias + alertas de sensores generales de AMBAS bombas
const alertasPorBomba = computed(() => {
  const alertasPropias = (alertasPorBombaRaw.value.B || []);

  if (!esBombaBActiva.value) {
    // Si Bomba B no está activa, quitar sensores generales
    return { ...alertasPorBombaRaw.value, B: alertasPropias.filter((a) => !esSensorGeneral(a.tipo_sensor)) };
  }

  // Bomba B está activa: agregar sensores generales de Bomba A también
  const generalesDeBombaA = (alertasPorBombaRaw.value.A || []).filter((a) => esSensorGeneral(a.tipo_sensor));
  const todasAlertasB = [...alertasPropias, ...generalesDeBombaA];

  // Deduplicar por sensor normalizado (flujo-agua-domo-ap y flujo_agua_domo_ap son el mismo)
  const vistos = new Set();
  const deduplicadas = todasAlertasB.filter((a) => {
  ...
  return { ...alertasPorBombaRaw.value, B: deduplicadas };
});

const sensoresConAnomaliasPorBomba = computed(() => {
  return {
    A: (alertasPorBombaRaw.value.A || []).map((a) => a.tipo_sensor).filter((v, i, arr) => arr.indexOf(v) === i),
    B: [...new Set(alertasPorBomba.value.B.map((a) => a.tipo_sensor))],
  };
});
...
const obtenerAlertaIdPorSensor = (sensorKey) => {
  const alertasBombaB = alertasPorBomba.value.B;
  const alerta = alertasBombaB.find(a => a.tipo_sensor === sensorKey);
...
const obtenerDescripcionPorSensor = (sensorKey) => {
  const alerta = alertasPorBomba.value.B.find(a => a.tipo_sensor === sensorKey);
  if (!alerta) return '';
...
const obtenerBombaOrigenPorSensor = (sensorKey) => {
  const alerta = alertasPorBomba.value.B.find(a => a.tipo_sensor === sensorKey);
  if (!alerta) return 'B';
  return (alerta.tabla_origen === 'A' || alerta.origen === 'Bomba A') ? 'A' : 'B';
};
...
const episodiosSensorPrincipal = computed(() => {
  ...
  return alertasPorBomba.value.B
