...
- **Paneles y análisis por voz**: "solo las críticas de la última semana", "siguiente página", "avisos de bitácoras" (filtros y paginación); "abre la crítica de corriente" (gráfico de esa alerta); "compara la corriente con la temperatura del estator", "agrega la presión de succión", "margen de 2 horas", "episodio anterior" (modo Conjunto).
- **Conocimiento de sensores** (`config/conocimientoSensores.ts`, adaptado de `SENSOR_INFO` y la base de conocimiento de Backend-GM): qué mide cada uno de los sensores, por qué importa, causas típicas y acción recomendada por nivel. Lo usan las alertas, el reporte, la herramienta `explicar_sensor` ("¿qué es la excentricidad?", "¿qué hago con la corriente?") y el analista LangGraph.
